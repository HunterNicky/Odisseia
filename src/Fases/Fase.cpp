...
namespace Fases{
    ...
    void Fase::newPlataforma(sf::Vector2f pos, sf::Vector2f size, const std::string path){
        Entidades::Obstaculos::Caixa* pCaixa = new Entidades::Obstaculos::Caixa(pos, size, Entidades::ID::Plataforma, path);
        pCaixa->setGerenciadorDeColisao(pColisao);
        LE.push_back(static_cast<Entidades::Entidade*>(pCaixa));
    }
    void Fase::newCaixa(sf::Vector2f pos, sf::Vector2f size, const std::string path){
    ...
    void Fase::atualizaPontuacao(){
        ...
        textoPontuacao.setPos(sf::Vector2f(posFundo.x - tamJan.x/4, posFundo.y - tamJan.y/2));
    ...
    void Fase::atualizaBarraDeVida(){
        sf::Vector2f posFundo = pGrafico->getViewCenter();
        sf::Vector2f tamJan = sf::Vector2f(1280.f, 720.f);
        barraDeVida->setPosition((sf::Vector2f(posFundo.x - tamJan.x/2 + 45.f, posFundo.y - tamJan.y/2 + 19.f)));
        tuboVida->setPosition(sf::Vector2f(posFundo.x - tamJan.x/2 , posFundo.y - tamJan.y/2));
        barraDeVida->setSize(sf::Vector2f((BARRA_VIDA_JOG_X - 50.f)*pJogador->getNum_vidas()/1000.f, BARRA_VIDA_JOG_Y -39.f));
