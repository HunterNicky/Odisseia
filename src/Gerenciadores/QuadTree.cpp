...
namespace Gerenciadores{
    ...
    void Quadtree::subdivide(){
        ...
        nodes[0] = new Quadtree(sf::FloatRect(center.x - halfSize.x, center.y - halfSize.y, halfSize.x, halfSize.y), capacity);
        nodes[1] = new Quadtree(sf::FloatRect(center.x, center.y - halfSize.y, halfSize.x, halfSize.y), capacity);
        nodes[2] = new Quadtree(sf::FloatRect(center.x - halfSize.x, center.y, halfSize.x, halfSize.y), capacity);
        nodes[3] = new Quadtree(sf::FloatRect(center.x, center.y, halfSize.x, halfSize.y), capacity);
    }
    ...
    void Quadtree::query(sf::FloatRect rect, std::vector<Entidades::Entidade*>& result){
    ...
    bool Quadtree::detectCollision(Entidades::Entidade* entidade, std::vector<Entidades::Entidade*>& colliEnti,sf::FloatRect& collisionRect, sf::Vector2f& mtv){
        ...
        for(auto& otherEntity : entities){
            if(otherEntity != entidade && otherEntity->getBody()->getGlobalBounds().intersects(entidade->getBody()->getGlobalBounds())){
                collisionRect = otherEntity->getBody()->getGlobalBounds();

                float overlapX = 
                std::min(collisionRect2.left + collisionRect2.width,collisionRect.left + collisionRect.width)
                - std::max(collisionRect2.left, collisionRect.left);

                float overlapY = 
                std::min(collisionRect2.top + collisionRect2.height,collisionRect.top + collisionRect.height)
                - std::max(collisionRect2.top, collisionRect.top);

                if(overlapX < overlapY){
                    mtv.x = (collisionRect2.left + collisionRect2.width / 2.f < collisionRect.left + collisionRect.width / 2.f) ? 
                    -overlapX : overlapX;
                }else{
