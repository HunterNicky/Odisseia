...
void Texto::setSize(const sf::Vector2f size) { this->size = size; }
...
void Texto::setTamanhoBorda(const float tamBorda) {
  ...
  size = sf::Vector2f(sfTexto.getGlobalBounds().width,
                      sfTexto.getGlobalBounds().height);
}
...
const sf::Vector2f Texto::getSize() const { return size; }

const sf::Text *Texto::getSfTexto() const { return &sfTexto; }

const std::string Texto::getText() const { return texto; }
} // namespace Botoes
} // namespace Menu
