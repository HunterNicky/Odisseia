...
MenuGameOver::MenuGameOver(Fases::Fase *pFase)
    : Menu(3, 2,
      ...
      pontuacao(sf::Vector2f(POS_PONTUACAO_X, POS_PONTUACAO_Y),
                sf::Vector2f(TAMANHO_GO_X, TAMANHO_GO_Y), "Pontuacao", 35) {
  inicializaBotao();
  ...
  if (!textura->loadFromFile("data/Botao/CaixaTexto.png")) {
    std::cout << "Jungle::Menu::MenuGameOver::nao foi possivel carregar textura"
              << std::endl;
    exit(1);
  }
  fundoCaracter->setTexture(textura);
  fundoCaracter->setSize(sf::Vector2f(400.f, 70.f));
  fundoCaracter->setPosition(
      sf::Vector2f(650.f - fundoCaracter->getSize().x / 2.0f, 200.f));
  ...
  texto.setPos(sf::Vector2f(
      posCaixa.x + tamCaixa.x / 2.0f - texto.getSize().x / 2.0f + 40.f,
      posCaixa.y + tamCaixa.y / 2.0f - texto.getSize().y / 2.0f + 5.f));
  pontuacao.setPos(sf::Vector2f(posCaixa.x + tamCaixaPontuacao.x / 2.0f -
                                    pontuacao.getSize().x / 2.0f - 80.f,
                                posCaixa.y + tamCaixaPontuacao.y / 2.0f -
                                    pontuacao.getSize().y / 2.0f + 80.0f));
}

MenuGameOver::~MenuGameOver() {}

void MenuGameOver::setFase(Fases::Fase *pFase) { this->pFase = pFase; }
void MenuGameOver::inicializaBotao() {
  Botoes::Botao *pBotao = NULL;

  pBotao = new Botoes::Botao(sf::Vector2f(640.f - 100, 360.f),
                             std::string(" Salvar Colocacao"));
